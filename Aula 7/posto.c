...
Posto* inicPosto(char* nome)
{
    ...
    posto -> amostrasNaoProcessadas = criaFila();
...
{
    printf("Dados do Posto: %s", posto -> nome);
    printf("Amostras não processadas\n");
    imprimeFila(posto -> amostrasNaoProcessadas);
...
void realizaColeta(Posto* posto, char* pessoa, int idade)
{
    ...
    if(idade >= 60)
        insereComecoFila(posto -> amostrasNaoProcessadas, amostra);
    else
        insereFila(posto -> amostrasNaoProcessadas, amostra);
}
...
{
    desalocaFila(posto -> amostrasNaoProcessadas);
    desalocaFila(posto -> Positivo);
    desalocaFila(posto -> Negativo);
    free(posto -> nome);
    free(posto);
}
